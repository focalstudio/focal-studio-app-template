...
`init.sh` applies `.github/labels.tsv` when it creates the repo, so `gh label list` should show 22 labels. If it shows fewer (label sync failed, or the repo pre-dates #127), re-apply it before creating issues — otherwise every `--label` below is rejected:
