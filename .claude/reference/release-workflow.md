...
4b. **Triage `PARKING.md`**: file each `## Parked` entry as an issue (type + priority + milestone labels, plus `parked`) or drop it with a one-line reason, and move it to `## Triaged` with the issue number or the reason. Then move the `next-release` label: open issues planned for the following release get it. See "One issue per session" in [AGENTS.md](../../AGENTS.md).
