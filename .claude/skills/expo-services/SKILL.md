...
**On Supabase, those printed manual steps are themselves scripted.** `bash scripts/provision-supabase.sh "App Name"` (with `SUPABASE_ACCESS_TOKEN` prefixed onto the command, never stored) creates the project via the Management API, writes `.env.local`, applies `schema.sql`, and verifies RLS + the `delete_own_account()` / `keepalive_ping()` grants actually took. Point the user at it rather than walking them through the dashboard. It cannot create their Supabase *account* — signup has no API. **Firebase has no equivalent and should not get one**: its non-interactive auth needs a service account that itself needs a pre-existing GCP project, and `projects:create` hits per-account quota and billing gates. Do not offer to build it.
