...
| JDK (Maestro runtime) | `17` | Maestro is a JVM app. CI pins it via `actions/setup-java` in [.github/workflows/maestro-e2e.yml](.github/workflows/maestro-e2e.yml); locally any JDK 17+ works, but `JAVA_HOME` must be exported — Maestro's launcher reads that variable specifically and a `java` on `PATH` alone is not enough. |
